...
export async function getInterventionRecommendations(classifiedInput: any) {
  try {
    // Mock database results - structured interventions with IRC references
    ...
    const rankedInterventions = scoredInterventions.map((scored, idx) => ({
      ...
      why_matched: `Matches ${classifiedInput.road_context?.road_type || 'road safety'} context with focus on ${classifiedInput.crash_issue?.symptoms?.[0] || 'crash prevention'}. Evidence base supports 85%+ effectiveness in similar settings.`,
    ...
    throw error instanceof Error ? error : new Error('Failed to generate recommendations')
  }
}
